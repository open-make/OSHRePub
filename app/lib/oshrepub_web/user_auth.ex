...
defmodule OSHRePubWeb.UserAuth do
  use OSHRePubWeb, :verified_routes
  ...
  @max_age 60 * 60 * 24 * 30
  @remember_me_cookie "_oshrepub_account_remember_me"
  ...
  """
  def log_in_to_account(conn, account, params \\ %{}) do
    token = Accounts.generate_session_token(account)
  ...
  """
  def log_out_of_account(conn) do
    user_token = get_session(conn, :user_token)
    user_token && Accounts.delete_session_token(user_token)

    if live_socket_id = get_session(conn, :live_socket_id) do
      OSHRePubWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  def fetch_current_user(conn, _opts) do
    ...
    account = token && Accounts.get_account_by_session_token(token)
  ...
  ## `on_mount` arguments
    ...
    * `:mount_current_account` - Assigns current_account
      ...
      there's no user_token or no matching account.

    * `:ensure_authenticated` - Authenticates the user from the session,
      and assigns the current_account to socket assigns based
      on user_token.
      Redirects to login page if there's no account logged in to.

    * `:redirect_if_user_is_authenticated` - Authenticates the user from the session.
      Redirects to signed_in_path if there's a account logged in to.
  ...
  the current_account:
      ...
      defmodule OSHRePubWeb.PageLive do
        use OSHRePubWeb, :live_view

        on_mount {OSHRePubWeb.UserAuth, :mount_current_account}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{OSHRePubWeb.UserAuth, :ensure_authenticated}] do
  ...
  If you want to enforce an account's email has to be confirmed before
  it can be used, here would be a good place.
  """
