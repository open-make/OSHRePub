...
defmodule OSHRePub.Accounts.UserToken do
  use Ecto.Schema
  import Ecto.Query
  alias OSHRePub.Accounts.UserToken
  ...
  def build_session_token(account) do
    ...
    {token, %UserToken{token: token, context: "session", account_id: account.id}}
  end
  ...
  the token in the application to gain access. Furthermore, if a user changes
  their account's email in the system, the tokens sent to the previous email are no longer
  valid.
  ...
  database and the account's email has not changed. This function also checks
  ...
  This is used to validate requests to change the account's
  ...
  def by_account_and_contexts_query(account, :all) do
    from t in UserToken, where: t.account_id == ^account.id
  end

  def by_account_and_contexts_query(account, [_ | _] = contexts) do
    from t in UserToken, where: t.account_id == ^account.id and t.context in ^contexts
  end
end
