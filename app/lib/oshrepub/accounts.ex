...
defmodule OSHRePub.Accounts do
  @moduledoc """
  ...
  alias OSHRePub.Accounts.{Account, UserToken, UserNotifier}
  ...
  defp account_email_multi(account, email, context) do
    ...
    |> Ecto.Multi.delete_all(:tokens, UserToken.by_account_and_contexts_query(account, [context]))
  end

  @doc ~S"""
  Delivers the update email instructions for the given account.

  ## Examples

      iex> send_account_update_email_instructions(account, current_email, &url(~p"/settings/confirm_email/#{&1})")
      {:ok, %{to: ..., body: ...}}

  """
  def send_account_update_email_instructions(%Account{} = account, current_email, update_email_url_fun)
      when is_function(update_email_url_fun, 1) do
    {encoded_token, user_token} = UserToken.build_email_token(account, "change:#{current_email}")

    Repo.insert!(user_token)
    UserNotifier.send_update_email_instructions(account, update_email_url_fun.(encoded_token))
  end
  ...
  def update_account_password(account, password, attrs) do
    ...
    Ecto.Multi.new()
    |> Ecto.Multi.update(:account, changeset)
    |> Ecto.Multi.delete_all(:tokens, UserToken.by_account_and_contexts_query(account, :all))
    |> Repo.transaction()
    |> case do
  ...
  def generate_session_token(account) do
    {token, user_token} = UserToken.build_session_token(account)
    Repo.insert!(user_token)
    token
  end

  @doc """
  Gets the account for the given signed token.
  """
  ...
  ## Examples
      ...
      iex> send_account_confirmation_instructions(account, &url(~p"/confirm_account/#{&1}"))
      {:ok, %{to: ..., body: ...}}

      iex> send_account_confirmation_instructions(confirmed_user, &url(~p"/confirm_account/#{&1}"))
      {:error, :already_confirmed}

  """
  def send_account_confirmation_instructions(%Account{} = account, confirmation_url_fun)
    ...
    if account.confirmed_at do
      ...
      Repo.insert!(user_token)
      UserNotifier.send_confirmation_instructions(account, confirmation_url_fun.(encoded_token))
    end
  end
  ...
  defp confirm_account_multi(account) do
    ...
    |> Ecto.Multi.delete_all(:tokens, UserToken.by_account_and_contexts_query(account, ["confirm"]))
  end

  ## Reset password

  @doc ~S"""
  Delivers the reset password email for the given account.

  ## Examples

      iex> send_account_reset_password_instructions(account, &url(~p"/reset_password/#{&1}"))
      {:ok, %{to: ..., body: ...}}

  """
  def send_account_reset_password_instructions(%Account{} = account, reset_password_url_fun)
      when is_function(reset_password_url_fun, 1) do
    {encoded_token, user_token} = UserToken.build_email_token(account, "reset_password")
    Repo.insert!(user_token)
    UserNotifier.send_reset_password_instructions(account, reset_password_url_fun.(encoded_token))
  end
